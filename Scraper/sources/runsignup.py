...
def fetch_runsignup_events(max_results: int = 100) -> List[RaceEvent]:
    """
    ...
    try:
        ...
        for race_data in races:
            ...
            event = RaceEvent(
                id=RaceEvent.generate_id(name, next_date),
                name=name,
                date=next_date,
                ...
                discipline="Running",
                distance=distance,
                elevationGain=None,  # Not provided by API
                description=race.get("description", f"Running event in {city}")[:200] if race.get("description") else f"Running event in {city}",
                registrationUrl=reg_url,
                imageUrl=name.lower().replace(" ", "_"),
                price=None,  # Would need separate API call
                currency=None,
        ...
        print(f"❌ RunSignup API error: {e}")
    
    # If we got few events, add fallback major marathons
    if len(events) < 5:
        events.extend(get_fallback_marathons())
    
    print(f"✅ RunSignup: Fetched {len(events)} events")
    return events
...
def get_fallback_marathons() -> List[RaceEvent]:
    ...
    marathons = [
        ...
        ("Fes Marathon", "2026-02-15", "Fes", "Morocco", "MA", "Imperial city historical run"),
    ...
    return [
        RaceEvent(
            ...
            registrationUrl=f"https://www.google.com/search?q={name.replace(' ', '+')}+registration",
            imageUrl=name.lower().replace(" ", "_"),
